  ...
  <div class="min-h-screen bg-gray-50">
    ...
    @endif
      ...
      <h3 class="text-sm font-medium text-gray-900">
        <a href="{{ route('shops.show', $item->product->slug) }}" class="hover:text-gray-600">
        {{ $item->product->name }}
        </a>
      </h3>
      ...
      <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center">
        ...
        max="{{ $item->product->stock }}" class="w-12 text-center border-0 focus:ring-0 text-sm"
      ...
      <div class="border-t border-gray-200 pt-4">
        ...
        <span class="text-base font-medium text-gray-900">${{ number_format($cart->subtotal, 2) }}</span>
        </div>
      </div>
      </div>
