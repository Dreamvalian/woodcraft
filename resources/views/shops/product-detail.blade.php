        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="mb-8">
                <ol class="flex items-center space-x-2 text-sm">
                    ...
                    <li><a href="{{ route('products.index') }}" class="text-[#7f8c8d] hover:text-[#E67E22]">Products</a>
                    </li>
                    <li class="text-[#7f8c8d]">/</li>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-6">
                    ...
                    <div class="space-y-4">
                        <div class="relative aspect-square rounded-lg overflow-hidden">
                            ...
                            @if($product->sale_price)
                                <div
                                    class="absolute top-4 right-4 bg-[#E67E22] text-white px-3 py-1 rounded-full text-sm font-medium">
                                    Sale
                                </div>
                            @endif
                            <button onclick="toggleZoom()"
                                class="absolute bottom-4 right-4 bg-white p-2 rounded-full shadow-lg">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#2C3E50]" fill="none"
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                <div class="aspect-square rounded-lg overflow-hidden cursor-pointer border-2 border-[#E67E22]">
                                    <img src="{{ $product->image_url }}" alt="{{ $product->name }}"
                                        onclick="changeMainImage(this.src)" class="w-full h-full object-cover">
                                </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-baseline space-x-4">
                            @if($product->sale_price)
                                <span
                                    class="text-2xl font-bold text-[#E67E22]">${{ number_format($product->sale_price, 2) }}</span>
                                <span
                                    class="text-lg text-[#7f8c8d] line-through">${{ number_format($product->price, 2) }}</span>
                            @else
                        ...
                        <form action="{{ route('cart.add', $product) }}" method="POST" class="space-y-4" id="addToCartForm">
                            @csrf
                            <div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex items-center border border-gray-300 rounded-lg">
                                        <button type="button" onclick="decrementQuantity()"
                                            class="px-4 py-2 text-[#7f8c8d] hover:text-[#E67E22] disabled:opacity-50 disabled:cursor-not-allowed">-</button>
                                ...
                                <p id="quantityError" class="text-red-600 text-sm mt-1 hidden"></p>
                            </div>

                            @if($product->stock > 0)
                                <button type="submit" id="addToCartButton"
                                    class="w-full bg-[#E67E22] text-white px-6 py-3 rounded-lg shadow hover:bg-[#2C3E50] transition disabled:opacity-50 disabled:cursor-not-allowed">
                            ...
                            @else
                                <button type="button" disabled
                                    class="w-full bg-gray-400 text-white px-6 py-3 rounded-lg shadow cursor-not-allowed">
                                    Out of Stock
                                </button>
                            @endif
                        ...
                        <div id="reviewsContainer" class="space-y-6">
                            @foreach($product->reviews as $review)
                                <div class="border-b border-gray-200 pb-6 last:border-0 review-item"
                                    data-date="{{ $review->created_at->timestamp }}" data-rating="{{ $review->rating }}">
                        ...
                        <div class="mt-8">
                            ...
                            @if(Route::has('reviews.store'))
                                <form action="{{ route('reviews.store', $product) }}" method="POST" class="space-y-4">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            @for($i = 1; $i <= 5; $i++)
                                                <button type="button" onclick="setRating({{ $i }})"
                                                    class="text-2xl text-gray-300 hover:text-[#E67E22] rating-star">
                                                    ★
                                                </button>
                                            @endfor
                                        </div>
                                    ...
                                    <button type="submit"
                                        class="bg-[#E67E22] text-white px-6 py-2 rounded-lg shadow hover:bg-[#2C3E50] transition">
                                        Submit Review
                                    </button>
                                </form>
                            @else
                                <p class="text-[#7f8c8d]">Review functionality is currently unavailable. Please check back later.
                                </p>
                            @endif
                        </div>
                    @else
                        <div class="mt-8 text-center">
                            <p class="text-[#7f8c8d]">Please <a href="{{ route('login') }}"
                                    class="text-[#E67E22] hover:underline">login</a> to write a review.</p>
                        </div>
                    @endauth
                </div>
                    ...
                    <div class="border-t border-gray-200 p-6">
                        ...
                        <div class="grid md:grid-cols-4 gap-6">
                            @foreach($relatedProducts as $relatedProduct)
                                <div
                                    class="group bg-white rounded-lg shadow-sm overflow-hidden transition-all duration-300 hover:shadow-lg">
    ...
    @push('scripts')
        <script>
            ...
            function validateQuantity(input) {
                ...
                if (value < min) {
                    input.value = min;
                    errorElement.textContent = `Minimum quantity is ${min}`;
                    errorElement.classList.remove('hidden');
                    submitButton.disabled = true;
                } else if (value > max) {
                    input.value = max;
                    errorElement.textContent = `Maximum quantity is ${max}`;
                    errorElement.classList.remove('hidden');
                    submitButton.disabled = true;
                } else {
